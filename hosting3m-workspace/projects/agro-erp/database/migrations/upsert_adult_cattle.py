...
def main():
    ...
    with open(path, newline="", encoding="utf-8") as f:
        ...
        for i, row in enumerate(reader, start=2):
            ...
            if tipo not in ("SEMENTAL", "VACA"):
                errors.append(f"Fila {i}: tipo '{row.get('tipo')}' debe ser SEMENTAL o VACA")
                continue
    ...
    # SQL generation
    # -----------------------------------------------------------------------
    id_company = COMPANY_ALIASES["el triunfo"]
    sql = [
    ...
    for r in rows_ok:
        ...
        category = "TORO" if r["tipo"] == "SEMENTAL" else "VACA"
        ...
        sql.append(
            ...
            "current_status, species, current_weight_kg, brand_id, leased_site_id, metadata) VALUES ("
            f"{id_company}, {sql_quote(r['rfid'])}, {sql_quote(r['numero_fuego'])}, "
            f"{sql_quote(category)}, {sql_quote(business_model)}, 'ACTIVO', 'BOVINO', 0, "
            f"{brand_subq}, {leased_site_subq}, {metadata_literal}"
            ") "
        ...
        if r["tipo"] == "VACA" and r["diagnostico"]:
            ...
            sql.append(
                ...
                f"{sql_quote('Palpación transcrita de libreta de campo — Rancho El Triunfo')}, "
