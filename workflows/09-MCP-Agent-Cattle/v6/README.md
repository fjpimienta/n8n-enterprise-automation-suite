...
### 1. 🔴 CRÍTICO — no existe ningún gate de rol verificado en NINGÚN canal (corregido este commit — tercer diagnóstico erróneo, ver `CLAUDE.md`)
La versión anterior de este README describía un nodo `Switch Role` en WhatsApp que bloqueaba a no-ADMIN antes de llegar al `AI Agent`, **y además afirmaba que el Web Chat sí compensaba eso con RBAC a nivel de prompt.** Ambas partes de esa afirmación están mal:
* *WhatsApp:* el flujo real es `Get User Role` (Postgres) → `Set Role` (guarda `role = global_role || 'GUEST'`) → `Resolver Tenant` → `AI Agent` — el campo `role` se calcula pero nunca se vuelve a leer en ningún nodo `IF` ni en el `systemMessage`.
* *Web Chat:* **confirmado ahora contra el `systemMessage` real de `v6_ai_chat_cattle`** (export pegado por el usuario, 2026-09-19) — no existe ninguna sección de RBAC. El prompt no menciona `role`, `ADMIN` ni `EDITOR` en ningún lado; solo cubre inyección de `tenant_id`/`user_email`, el protocolo de confirmación de mortandad/venta, zero-hallucination, formato de transmisión de parámetros y el diccionario de herramientas. Lo que el README describía antes (bloqueo de las 7 tools de escritura para no-ADMIN) **no existe en el prompt actual** — ya sea que nunca existió y se documentó mal, o que existió en una versión anterior del prompt y se perdió al editarlo. Ninguna de las dos hipótesis es verificable desde aquí sin ver el historial de versiones del workflow en n8n.

**Consecuencia real y verificada:** un usuario con rol EDITOR (con acceso legítimo a un tenant real) puede hoy invocar las 7 herramientas de escritura — incluyendo `log_mortality_event`/`request_livestock_sale` — **por cualquiera de los dos canales**, sin que ningún nivel (n8n, prompt o SQL, ver punto 2) se lo impida. Esto ya no es un problema exclusivo de WhatsApp: es un problema de todo el subsistema del Agente IA. El cálculo de `global_role` en WhatsApp sin uso posterior sigue sugiriendo que un gate existió y se perdió, pero ahora no hay ningún canal de referencia que muestre cómo se veía ese gate funcionando. **Pendiente de decisión del equipo:** diseñar el gate de RBAC (a nivel de nodo n8n, de prompt, o ambos) desde cero para los dos canales, no solo "replicar" uno que ya no existe en ninguno.

### 2. RBAC a nivel SQL — parcial, no lo que se documentaba antes
Solo `get_livestock_info` (lectura), `log_cattle_weight` y `log_health_event` validan pertenencia al tenant vía `JOIN user_companies`/`users`/`is_active` en el propio nodo Postgres. **Ninguna de las 10 tools valida `role` en SQL.** `register_ranch_expense` y `find_calf_by_dam` son las más expuestas: la primera sin chequeo de rol (ya documentado), la segunda sin chequeo de tenant siquiera (ver arriba). Las tools que invocan un SP (`log_weaning_event`, `register_birth_event`, `log_mortality_event`, `request_livestock_sale`) no muestran validación en el nodo — si existe, vive dentro del SP y no es verificable desde este archivo. **Con el punto 1 corregido, este es hoy el único nivel de defensa real contra un EDITOR** (y solo para 3 de las 10 tools).

### 3. RBAC a nivel de prompt — ninguno confirmado en ningún canal (antes decía "Web Chat únicamente"; corregido)
Ni el `systemMessage` de WhatsApp ni el de `v6_ai_chat_cattle` condicionan ninguna herramienta al valor de `role`. El protocolo de confirmación (mortandad/venta) y el bloqueo de `tenant_id` inventado siguen presentes en ambos — pero ninguno de los dos es un control de *rol*, son controles de *tenant* y de *irreversibilidad*, no de *quién* puede hacer la operación. `find_calf_by_dam` y `get_table_metadata` tampoco aparecen mencionadas por rol en ninguno de los dos prompts, consistente con que no hay lista de bloqueo/permitidas de ningún tipo.
