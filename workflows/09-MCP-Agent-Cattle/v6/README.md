...
### 1. ✅ Ausencia de gate de rol — re-alcanzada y cerrada como diseño intencional (decisión de Francisco Pérez Pimienta, PM, 2026-09-20; NO confirmada todavía con el cliente final)
La versión anterior de este README describía un nodo `Switch Role` en WhatsApp que bloqueaba a no-ADMIN antes de llegar al `AI Agent`, y afirmaba que Web Chat compensaba eso con RBAC a nivel de prompt — **ambas partes de esa afirmación eran falsas**, confirmado contra el `systemMessage` real de `v6_ai_chat_cattle` (2026-09-19): ningún canal condiciona ninguna herramienta al valor de `role`.

Al re-plantear el hallazgo como decisión de producto en vez de solo como bug, la lectura correcta es: **el Agente IA (WhatsApp/Web Chat) es una herramienta de captura de campo — EDITOR debe poder usar las 5 tools rutinarias** (`log_cattle_weight`, `log_health_event`, `register_ranch_expense`, `log_weaning_event`, `register_birth_event`) **sin restricción de rol, por diseño.** Las dos operaciones genuinamente irreversibles (`log_mortality_event`/`request_livestock_sale`) **ya tienen su gate real**, vía el protocolo de autorización asíncrona (Regla 10 de `CLAUDE.md`): cualquier solicitud, la haga ADMIN o EDITOR, queda `PENDIENTE` hasta que un ADMIN del tenant la apruebe desde `/admin/autorizaciones`. Bajo esta lectura, **no hace falta construir ningún gate de rol nuevo** — el `Set Role`/`global_role` de WhatsApp queda como cálculo sin consumidor (deuda cosmética, no de seguridad), y el README anterior estaba documentando un diseño más estricto que nunca fue la intención real del producto.
⚠️ **Esta decisión la tomó el PM del proyecto, no el cliente final** — a diferencia de otras decisiones de negocio de este archivo (ej. Regla 10, confirmada con Alejandro), esta todavía no tiene una confirmación explícita registrada del lado del cliente. Tratarla como el diseño vigente para efectos de desarrollo, pero **no cerrarla en `CLAUDE.md` como "confirmado con el cliente"** hasta que exista esa confirmación.

**Lo que sigue sin gate, y por qué es aceptable bajo esta decisión:** un EDITOR sin `role` reconocido (`GUEST`, teléfono no asociado a ningún `user_companies`) no llega a ejecutar tools de todos modos, porque `Resolver Tenant` no encuentra tenant para él y `IF - Bloquear Tenant Ambiguo` corta el flujo antes del `AI Agent` (ver punto 5) — el control real que importa hoy no es *rol*, es *pertenencia al tenant*, y ese sí existe (aunque solo a nivel SQL en 3 de las 10 tools, ver punto 2).

### 2. RBAC a nivel SQL — parcial, y suficiente para el diseño confirmado en el punto 1
Solo `get_livestock_info` (lectura), `log_cattle_weight` y `log_health_event` validan pertenencia al tenant vía `JOIN user_companies`/`users`/`is_active` en el propio nodo Postgres. **Ninguna de las 10 tools valida `role` en SQL — y con el punto 1 no debería, salvo para `log_mortality_event`/`request_livestock_sale`, que ya están cubiertas por Regla 10 en el SP, no por SQL de la tool.** `register_ranch_expense` y `find_calf_by_dam` siguen siendo las más expuestas *en tenant*, no en rol: la primera sin chequeo de tenant explícito en el nodo, la segunda sin `JOIN user_companies`/`users` (ver Seguridad, punto 7, hallazgo v1.12.0) — ese sigue siendo el hallazgo real pendiente de resolver, no el de rol.

### 3. RBAC a nivel de prompt — ninguno en ningún canal, y no se necesita uno nuevo (antes decía "Web Chat únicamente"; corregido)
Ni el `systemMessage` de WhatsApp ni el de `v6_ai_chat_cattle` condicionan ninguna herramienta al valor de `role` — consistente con la decisión del punto 1. El protocolo de confirmación (mortandad/venta) y el bloqueo de `tenant_id` inventado siguen presentes en ambos, pero son controles de *tenant* e *irreversibilidad*, no de *rol*. `find_calf_by_dam` y `get_table_metadata` tampoco aparecen mencionadas por rol en ninguno de los dos prompts, consistente con que no hay lista de bloqueo/permitidas de ningún tipo — y no hace falta agregarla.
...
* *Web Chat*: la sub-rutina `v3/SW ValidaToken` (workflow `RSz6L3aXj3NfumwG`) valida el `Authorization` header y resuelve `tenant_id`/rol a partir del token. **Confirmado contra el export real del sub-workflow (no documentado hasta ahora):** es una rutina **compartida entre módulos del monorepo** (recibe `model_name` como parámetro), no exclusiva de `agro-erp` — antes de tocar `Verify Token` pasa por un nodo `Bypass para Reseñas?` que, si `model_name === 'hotel_reviews'`, responde de inmediato con un gafete hardcodeado (`{ id_company: 1, role: 'PUBLIC', user: 'web_guest' }`) **sin validar ningún token**, pensado para que un visitante anónimo de `hotel-website` pueda publicar una reseña sin login. Para cualquier otro `model_name` (incluido el que manda `v6_ai_chat_cattle`) sigue por `Verify Token` (`POST http://jwt-service:4000/verify-token`, con `Authorization` reenviado e `internal_secret` inyectado desde `$env["INTERNAL_SECRET"]`, `onError: continueRegularOutput`) y solo arma el gafete real (`Send Token`) si `valid === true`; si no, corta con `Stop and Error`. **Sin impacto de seguridad conocido sobre el canal de ganado** — la llamada de `v6_ai_chat_cattle` a esta sub-rutina nunca manda `model_name = 'hotel_reviews'` — pero confirma que el bypass público es una decisión de diseño explícita del gateway compartido, no un descuido aislado de `agro-erp`; queda como referencia por si algún flujo futuro de este módulo reutiliza la misma sub-rutina con un `model_name` incorrecto.
